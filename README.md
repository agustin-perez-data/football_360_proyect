...
4) Create analytical views
	 - Run from the Postgres container (the `sql` folder is mounted there):
		 ```bash
		 docker exec football_db bash -lc "export PGPASSWORD=football; psql -U football -d football -f /sql/views.sql"
		 ```
...
- If you prefer running scripts on your host Python:
	...
	3) Set environment variables (Windows PowerShell example):
		 ...
		 $env:POSTGRES_USER = "football"
		 $env:POSTGRES_PASSWORD = "football"
...
- psql inside Postgres: `docker exec -it football_db psql -U football -d football`
...
- Permission or auth errors:
	- Confirm credentials: user `football`, password `football`, db `football`.
- Views not found:
	- Re-run: `docker exec football_db bash -lc "export PGPASSWORD=football; psql -U football -d football -f /sql/views.sql"`
- Fresh start:
